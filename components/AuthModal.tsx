...
export default function AuthModal({ isOpen, onClose, onAuthSuccess }: AuthModalProps) {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="text-right mt-2">
          <button type="button" onClick={() => { setMode('forgotPassword'); setAuthError(''); setForgotPasswordError(''); setForgotPasswordMessage(''); }} className="text-gray-400 hover:text-gray-300 text-sm">
  ...
  const renderForgotPasswordForm = () => (
    <>
      <p className="text-center text-gray-400 text-sm">
        Lembrou da senha? <button onClick={() => { setMode('login'); setForgotPasswordError(''); setForgotPasswordMessage(''); }} className="text-white underline">Inicie sessão aqui</button>
      </p>
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="w-full max-w-xl bg-black border border-gray-700">

          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700">
            <div className="flex items-center gap-6 text-sm">
              {/* Criar conta */}
              <button onClick={() => setMode('signup')} className={`flex items-center gap-2 ${mode === 'signup' ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-300'}`}>
              ...
              {/* Log in */}
              <button onClick={() => setMode('login')} className={`flex items-center gap-1 ${mode === 'login' ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-300'}`}>
              ...
              {/* Esqueci-me */}
              <button onClick={() => setMode('forgotPassword')} className={`flex items-center gap-1 ${mode === 'forgotPassword' ? 'text-white font-medium' : 'text-gray-400 hover:text-gray-300'}`}>
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
